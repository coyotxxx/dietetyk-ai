...
/**
 * v16→v17: profil rozszerzony o alergie strukturalne + typ diety + kadencję różnorodności.
 * `allergens`/`dietType` nullable (TEXT), `varietyMode` NOT NULL DEFAULT 'SAME_DAILY' — zgodne ze
 * schematem generowanym przez Room (String?→TEXT, String z defaultem→TEXT NOT NULL DEFAULT). ADD COLUMN,
 * nie-destrukcyjne, zero utraty danych.
 */
val MIGRATION_16_17 = object : Migration(16, 17) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `profile` ADD COLUMN `allergens` TEXT")
        db.execSQL("ALTER TABLE `profile` ADD COLUMN `dietType` TEXT")
        db.execSQL("ALTER TABLE `profile` ADD COLUMN `varietyMode` TEXT NOT NULL DEFAULT 'SAME_DAILY'")
    }
}
